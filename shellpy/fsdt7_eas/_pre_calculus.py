...
from shellpy.fsdt7_eas._auxiliear_math import calc_nl_numba, calc_Kuu_numba
...
def cache_last_u(func):
    """
    Cache specialized for functions whose only changing argument is `u`.

    Stores only the last evaluation to avoid memory growth and expensive hashing.
    """
...
def _pre_calculus2(
    ...
    nl0 = calc_nl_numba(epsilon0_nl, u)
    nl1 = calc_nl_numba(epsilon1_nl, u)
    nl2 = calc_nl_numba(epsilon2_nl, u)
    ...
    L0 = (
            np.einsum('abxy,ibxy->iaxy', C0, E0, optimize=True) +
            np.einsum('abxy,ibxy->iaxy', C1, E1, optimize=True) +
            np.einsum('abxy,ibxy->iaxy', C2, E2, optimize=True)
    )

    L1 = (
            np.einsum('abxy,ibxy->iaxy', C1, E0, optimize=True) +
            np.einsum('abxy,ibxy->iaxy', C2, E1, optimize=True) +
            np.einsum('abxy,ibxy->iaxy', C3, E2, optimize=True)
    )

    L2 = (
            np.einsum('abxy,ibxy->iaxy', C2, E0, optimize=True) +
            np.einsum('abxy,ibxy->iaxy', C3, E1, optimize=True) +
            np.einsum('abxy,ibxy->iaxy', C4, E2, optimize=True)
    )
    ...
    K_u_u = (
            np.einsum('maxy,naxy->mn', B0, L0 * Wxy1, optimize=True) +
            np.einsum('maxy,naxy->mn', B1, L1 * Wxy1, optimize=True) +
            np.einsum('maxy,naxy->mn', B2, L2 * Wxy1, optimize=True)
    )

    t5 = time.perf_counter()

    #K_u_u = (calc_Kuu_numba(B0, L0 * Wxy1) +
    #         calc_Kuu_numba(B1, L1 * Wxy1) +
    #         calc_Kuu_numba(B2, L2 * Wxy1))

    #t5a = time.perf_counter()

    K_u_alpha = (
            np.einsum('maxy,naxy->mn', B0, L0_alpha * Wxy1, optimize=True) +
            np.einsum('maxy,naxy->mn', B1, L1_alpha * Wxy1, optimize=True) +
            np.einsum('maxy,naxy->mn', B2, L2_alpha * Wxy1, optimize=True)
    )
