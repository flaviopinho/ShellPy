...
def stiffness_matrix(shell: Shell, n_x=20, n_y=20, n_z=10, integral_method=gauss_weights_simple_integral):
    # Get integration points and weights for the double integral over the mid-surface domain
    xi1, xi2, Wxy = double_integral_weights(shell.mid_surface_domain, n_x, n_y, integral_method)
    ...
    # a = 2 * xi1[:, 0] / (shell.mid_surface_domain.edges["xi1"][1] - shell.mid_surface_domain.edges["xi1"][0]) - 1
    # b = 2 * xi2[0, :] / (shell.mid_surface_domain.edges["xi2"][1] - shell.mid_surface_domain.edges["xi2"][0]) - 1
    # c = 2 * xi3[0, 0, :] / h[0, 0]
    # epsilon_alpha = enhanced_assumed_strain((a,b,c), strain_fields, 10, max_z)
    epsilon_alpha = enhanced_assumed_strain_new((xi1, xi2, xi3), shell.displacement_expansion, strain_fields, max_z)

    L_alpha = np.einsum('ijxyz, njxyz->nixyz', C[np.ix_(strain_fields, strain_fields)], epsilon_alpha, optimize=True)

    L_lin_z = (np.einsum('ijxyz, mjxy, xyz->mixyz', C[strain_fields], epsilon0_lin,
                         xi3 ** 0, optimize=True) +
               np.einsum('ijxyz, mjxy, xyz->mixyz', C[strain_fields], epsilon1_lin,
                         xi3, optimize=True) +
               np.einsum('ijxyz, mjxy, xyz->mixyz', C[strain_fields], epsilon2_lin,
                         xi3 ** 2, optimize=True))
    ...
    err_Kaa = np.linalg.norm(A - A.T)
    print("Asymmetry Kaa:", err_Kaa)

    print('Calculating quadratic strain energy functional...')
    start = time()
    stiffness_matrix = np.einsum('maxy, naxy, xy->mn', L0_lin, epsilon0_lin, Wxy1,
                                 optimize=True)
    stiffness_matrix += np.einsum('maxy, naxy, xy->mn', L1_lin, epsilon1_lin, Wxy1,
                                  optimize=True)
    stiffness_matrix += np.einsum('maxy, naxy, xy->mn', L2_lin, epsilon2_lin, Wxy1,
                                  optimize=True)
    stiffness_matrix = 0.5 * (stiffness_matrix + stiffness_matrix.T)
    stiffness_matrix -= K2
