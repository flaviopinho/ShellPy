...
@cache_function
def constitutive_tensor_for_fosd(mid_surface_geometry: MidSurfaceGeometry,
                                 material: IsotropicHomogeneousLinearElasticMaterial, xi1, xi2,
                                 xi3):
    metric_tensor_contravariant_components = mid_surface_geometry.metric_tensor_contravariant_components_extended(
        xi1, xi2)

    shifter_tensor_inverse = mid_surface_geometry.shifter_tensor_inverse_extended(xi1, xi2, xi3)

    metric_tensor2 = np.einsum('oaxyz, gbxyz, ogxy -> abxyz',
...
@cache_function
def constitutive_tensor_for_fosd(mid_surface_geometry: MidSurfaceGeometry, material: FunctionallyGradedMaterial, xi1,
                                 xi2,
                                 xi3):
    metric_tensor_contravariant_components = mid_surface_geometry.metric_tensor_contravariant_components_extended(
        xi1, xi2)
    ...
    print(np.shape(E))
    aux1 = E / (2.0 * (1.0 + nu))
    aux2 = E * nu / ((1.0 + nu) * (1.0 - 2.0 * nu))

    return np.einsum('xyz, ijklxyz->ijklxyz', aux1, Ta) + np.einsum('xyz, ijklxyz->ijklxyz', aux2, T3)
