...
from ..fsdt6.constitutive_matriz_in_material_frame import constitutive_matrix_in_material_frame
...
def strain_energy(shell: Shell, n_x=20, n_y=20, n_z=10, integral_method=gauss_weights_simple_integral):
    # Get integration points and weights for the double integral over the mid-surface domain
    xi1, xi2, Wxy = double_integral_weights(shell.mid_surface_domain, n_x, n_y, integral_method)
    ...
    L0_nl = np.zeros((n_dof, n_dof, 6) + n_xy)
    L1_nl = np.zeros((n_dof, n_dof, 6) + n_xy)
    L2_nl = np.zeros((n_dof, n_dof, 6) + n_xy)
    ...
    for i in range(n_dof):
        for j in range(i, n_dof):  # Compute only for i <= j to exploit symmetry
            ...
            L0_nl[i, j] = (np.einsum('ijxy, jxy->ixy', C0, epsilon0_nl[i, j], optimize=True) +
                           np.einsum('ijxy, jxy->ixy', C1, epsilon1_nl[i, j], optimize=True) +
                           np.einsum('ijxy, jxy->ixy', C2, epsilon2_nl[i, j], optimize=True))

            L1_nl[i, j] = (np.einsum('ijxy, jxy->ixy', C1, epsilon0_nl[i, j], optimize=True) +
                           np.einsum('ijxy, jxy->ixy', C2, epsilon1_nl[i, j], optimize=True) +
                           np.einsum('ijxy, jxy->ixy', C3, epsilon2_nl[i, j], optimize=True))

            L2_nl[i, j] = (np.einsum('ijxy, jxy->ixy', C2, epsilon0_nl[i, j], optimize=True) +
                           np.einsum('ijxy, jxy->ixy', C3, epsilon1_nl[i, j], optimize=True) +
                           np.einsum('ijxy, jxy->ixy', C4, epsilon2_nl[i, j], optimize=True))
    ...
    print('Calculating quadratic strain energy functional...')
    start = time()
    quadratic_energy_tensor = 0.5 * np.einsum('maxy, naxy, xy->mn', L0_lin, epsilon0_lin, Wxy1,
                                              optimize=True)
    quadratic_energy_tensor += 0.5 * np.einsum('maxy, naxy, xy->mn', L1_lin, epsilon1_lin, Wxy1,
                                               optimize=True)
    quadratic_energy_tensor += 0.5 * np.einsum('maxy, naxy, xy->mn', L2_lin, epsilon2_lin, Wxy1,
                                               optimize=True)
