...
def R(xi1_, xi2_):
    ...
    R = 0.6
    xi2_ = xi2_ + xi1_ / 2
    x = np.cos(xi1_ / np.sqrt(2)) - R * (np.cos(xi2_) * np.cos(xi1_ / np.sqrt(2)) + 1 / np.sqrt(2) * np.sin(xi2_) * np.sin(xi1_ / np.sqrt(2)))
    y = xi1_ / np.sqrt(2) - R / np.sqrt(2) * np.sin(xi2_)
    z = np.sin(xi1_ / np.sqrt(2)) - R * (np.cos(xi2_) * np.sin(xi1_ / np.sqrt(2)) - 1 / np.sqrt(2) * np.sin(xi2_) * np.cos(xi1_ / np.sqrt(2)))
...
thickness = 0.05  # Espessura da casca [m]
...
with open("malha_abaqus_spiral_tube2.inp", "w") as f:
    f.write("*HEADING\n")
