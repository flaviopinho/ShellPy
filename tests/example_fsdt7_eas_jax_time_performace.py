...
def setup_problem(order_u, order_eas):
    ...
    # Integration points (should grow if the polynomial order grows significantly)
    nx, ny, nz = 10, 10, 5
...
def test_fsdt7_eas_benchmark_and_accuracy():
    ...
    of the JAX JIT implementation against the classic NumPy implementation.
    """
    print("\n============================================================")
    print("   PERFORMANCE & ACCURACY TEST: Classic NUMPY vs JAX JIT      ")
    print("============================================================\n")
    ...
    for order in test_orders:
        ...
        # 1. Generate DISTINCT random vectors to simulate steps where 'u' changes.
        # This prevents NumPy's cache from artificially inflating performance.
        np.random.seed(42)
        ...
        # We only store the last iteration's results for validation to save memory
        F_np = None
        K_np = None
        for u in u_vectors:
        ...
        # CORREÇÃO AQUI: Desempacotando a tupla nas duas funções
        ...
        # Usamos a função 'get_f_and_k_jax' que retorna a força e a matriz tangente
        F_jax_first, K_jax_first = get_f_and_k_jax(jnp.array(u_vectors[0]))
        F_jax_first.block_until_ready()  # Force GPU/CPU to finish
        ...
        # Explicit Assertions
        # Using a slightly looser tolerance for the tangent matrix (K)
        # due to high-order terms captured exactly by JAX's automatic differentiation.
        ...
        print(f"  - NumPy Time     : {time_np:.5f} s / iteration")
        print(f"  - JAX JIT Time   : {time_jax_hot:.5f} s / iteration")
        print(f"  - JAX Setup      : Factory {time_factory:.2f}s | Compile {time_compile:.2f}s")
        print(f"  - ACCELERATION   : JAX Hot-Loop is {speedup:.2f}x faster!\n")


if __name__ == "__main__":
    # Allows the test to be run directly via `python script.py`
