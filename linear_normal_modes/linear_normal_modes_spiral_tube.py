...
if __name__ == "__main__":
    ...
    R_ = sym.Matrix([sym.cos(xi1_/np.sqrt(2)) - R * (sym.cos(xi2_) * sym.cos(xi1_/np.sqrt(2))+1/np.sqrt(2)*sym.sin(xi2_) * sym.sin(xi1_/np.sqrt(2))),
                      xi1_/np.sqrt(2) - R/np.sqrt(2)*sym.sin(xi2_),
                      sym.sin(xi1_/np.sqrt(2)) - R * (sym.cos(xi2_) * sym.sin(xi1_/np.sqrt(2))-1/np.sqrt(2)*sym.sin(xi2_) * sym.cos(xi1_/np.sqrt(2)))])
    ...
    for i in range(n_modes):
        ...
        mode = mode / np.max(np.abs(mode)) * 0.0  # Normalize and scale for visualization
